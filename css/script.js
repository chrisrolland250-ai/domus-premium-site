// Simulateur + accessibilité + scroll doux
(function(){
  ...
  function calc(){
    ...
    if (!eligible.checked){
      out.textContent = euro(m);
      note.textContent = 'Non éligible à l’Avance Immédiate : vous payez le montant total aujourd’hui.';
      return;
    }
  ...
  ['input','change','keyup'].forEach(evt => {
    montant.addEventListener(evt, calc);
    eligible.addEventListener(evt, calc);
  });
  calc();
})();
...
(function(){
  function smoothScroll(e){
    ...
    if (el){
      ...
      el.setAttribute('tabindex','-1'); // focus for a11y
